...
describe('dashboard', () => {
  ...
  test('lists all six issue types', async () => {
    const { body } = await get('/');
  ...
  test('lists all three transfer statuses', async () => {
    const { body } = await get('/');
...
describe('products screen', () => {
  ...
  test('flags the SKU that stock points at but the catalogue does not hold', async () => {
    const { body } = await get('/products');
...
describe('inventory audit screen', () => {
  ...
  test('the discrepancies-only filter keeps only the lines that disagree', async () => {
    const { shown, total } = shownCount((await get('/audit', 'difference=yes')).body);
    assert.ok(shown > 0 && shown < total);
